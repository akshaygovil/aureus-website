...
export default function AureusFeatureShowcase() {
  ...
  const oneRM =
    oneRMWeight && oneRMReps
      ? Math.round(Number(oneRMWeight) * (1 + Number(oneRMReps) / 30))
      : null;
  ...
  return (
    <section className="relative w-full min-h-screen py-28 bg-gradient-to-b from-[#FFFDF5] via-[#FAF8EF] to-[#F8F6EB] overflow-hidden flex flex-col items-center">
      {/* Glowing Background */}
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div className="absolute top-[10%] left-[10%] w-[700px] h-[700px] bg-[radial-gradient(circle_at_center,rgba(201,162,39,0.25),transparent_70%)] blur-[150px]" />
        <div className="absolute bottom-[10%] right-[10%] w-[600px] h-[600px] bg-[radial-gradient(circle_at_center,rgba(90,131,215,0.2),transparent_70%)] blur-[160px]" />
      </div>
      ...
      <motion.h2
        ...
        viewport={{ once: true }}
        className="text-4xl sm:text-5xl font-semibold text-[#1E1E1E] tracking-tight text-center mb-6"
      >
      ...
      <motion.p
        ...
        viewport={{ once: true }}
        className="text-lg sm:text-xl text-[#4B4B4B] max-w-2xl text-center mb-16 leading-relaxed"
      >
        Experience Aureus utilities directly — built to look like app screenshots, beautifully displayed in glass panels.
      </motion.p>

      {/* Panels */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 w-full max-w-6xl px-6">
        {/* --- 1RM CALCULATOR PANEL --- */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          viewport={{ once: true}}
          className="relative rounded-3xl bg-white/50 backdrop-blur-2xl border border-[#EDE7D2]/70 shadow-[0_8px_50px_rgba(0,0,0,0.08)] p-8 flex flex-col items-center"
        >
          <h3 className="text-2xl font-semibold mb-4 text-[#1E1E1E]">1RM Calculator</h3>
          <p className="text-[#4B4B4B]/80 mb-8 text-center">
            Predict your maximum strength with precision.
          </p>
          ...
          <div className="w-full max-w-sm mx-auto">
            <div className="flex flex-col gap-5 mb-6">
              <input
                ...
                className="w-full px-5 py-3 rounded-xl border border-[#E9E2C7] bg-white/80 placeholder:text-gray-400 text-[#1E1E1E] focus:outline-none focus:ring-2 focus:ring-[#C9A227]/40 transition"
              />
              <input
                type="number"
                placeholder="Reps"
            ...
            <div className="h-[80px] flex flex-col items-center justify-center rounded-xl bg-gradient-to-r from-[#F9E8A1] to-[#E3D28E] text-[#1E1E1E] shadow-[0_0_25px_rgba(201,162,39,0.25)]">
              {oneRM ? (
                <>
      ...
      >
        <div className="w-full max-w-md rounded-2xl bg-white shadow-[0_4px_50px_rgba(0,0,0,0.06)] border border-gray-100 p-6">
          <div className="rounded-xl border border-gray-200 bg-[#FAFAFA] shadow-inner p-5 mb-5 flex flex-col items-center">
            ...
            <div className="flex items-center justify-center gap-3">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-300 text-xl font-bold hover:bg-gray-100 transition"
              >
                –
              </button>
              ...
              <div className="flex items-end">
                <input
                  ...
                  className="w-20 text-center text-4xl font-bold text-[#1E1E1E] bg-transparent outline-none"
                />
                <span className="ml-1 text-sm text-gray-600">kg</span>
              </div>
            ...
            <p className="text-xs text-gray-500 mt-2">
              {maxExceeded ? `⚠️ Max capacity: ${barType === "standard" ? 300 : 470} kg` : isExact ? "Perfectly matched" : `Closest weight: ${achieved.toFixed(1)} kg`}
            </p>
          </div>

          {/* Bar Type Toggle */}
          <div className="flex rounded-lg bg-[#F9F9F9] p-1 mb-5 border border-gray-200">
            {(["standard", "powerlifting"] as const).map((t, i) => (
              <button
                ...
                className={`flex-1 py-2 text-sm font-medium rounded-md transition ${
                  barType === t
                    ? "bg-[#F3EAD0] text-[#1E1E1E] font-semibold"
                    : "text-gray-600 hover:text-[#BFA36F]"
          ...
          <div className="border border-gray-200 rounded-xl bg-[#FFFFFF] p-3 mb-5 shadow-[inset_0_0_15px_rgba(0,0,0,0.03)]">
              ...
              <motion.button
                ...
                className={`flex flex-col items-center rounded-lg border px-2 py-2 text-xs font-semibold transition ${
                  available[p.value] !== false
                    ? "border-[#C9A227]/40 bg-white shadow-[0_2px_10px_rgba(201,162,39,0.06)]"
              ...
              >
                ...
                {available[p.value] !== false && (
                  <motion.span
                    ...
                    className="text-emerald-500 text-[10px] font-bold"
                  >
                    ✓
                  </motion.span>
                )}
